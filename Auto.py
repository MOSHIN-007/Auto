...
class menu:
	...
	def tampilan_logo(self):
		...
		vp=Q
		jalan(f"""{xnxx}
{II}          _______  _______  _______  _______  _       
|\     /|(  ___  )(  ____ \(  ____ \(  ____ \( (    /|
( \   / )| (   ) || (    \/| (    \/| (    \/|  \  ( |
 \ (_) / | (___) || (_____ | (__    | (__    |   \ | |
  \   /  |  ___  |(_____  )|  __)   |  __)   | (\ \) |
 ...
 Author   : {P}YASEEN AFTAB {Q}
 Coded    : {P}YXT YASIN CODE{Q}
 Whatsapp : 923433978031
 Facebook : ALISH ALI ({II} Please Follow {Q})
 Github   : - Yasin1538
 """)
...
def login():
	...
	jalan(f"""{II}  ______  _______ __   _ _______ _______ _______
 |_____] |______ | \  | |_____| |_____| |  |  |
 |_____] |______ |  \_| |     | |     | |  |  |
                                                     {Q}
               
     FEEL KR : ("YASEEN X ALISH")
     LOVE U ("RAHEEL") 🥺💔
     CHL AB TOKEN DAL
...
def token_me(token):
	try:
		...
		jalan(f"[{M}!{Q}] Welcome {O}{nama}{Q}.... umeed krta ho tmhara akhri token hoga")
		...
		jalan(f" |-->Run Sript Again !!")
		time.sleep(1)
	except KeyError:
		os.system("rm -f .login.txt")
		jalan(f"[{M}!{Q}]-->Sorry Token Not Durable");os.sys.exit()
def cookie_me():
	cokex = input(" |-->Cookie : ")
